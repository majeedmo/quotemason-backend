...
- **Demolition & site prep**: both of Company A's standard scopes (§1) start from a basement that is *unfinished*, not previously finished — there is nothing to demolish just because it already has existing entrance/egress/rough-in infrastructure. Only include a demolition line when intake explicitly describes existing finished material to remove (e.g. "partially finished," "gut job," "tearing out old flooring/drywall/fixtures," a previous renovation being redone). Never add it by default or because a comparable past project happened to have one — that project may have been a gut job, this one may not be. [Added 2026-07-26 after confirming 3 of 4 identical-spec test quotes hallucinated a $2,450 demolition charge with no supporting intake detail]
- **Plumbing and HVAC rough-in/finish are baseline scope, never omitted entirely**: every finished basement or accessory unit needs *some* plumbing (at minimum, the wet bar/kitchen sink's supply and drain) and *some* HVAC (at minimum, ventilation/duct tie-in for the new living space) — these two trades must never be missing from a takeoff outright, even when no other specific line calls for them. [Added 2026-07-26 after confirming HVAC and/or plumbing were silently dropped entirely from 2 of 4 identical-spec test quotes; also enforced deterministically in code as a backstop, not left to prompt compliance alone]
